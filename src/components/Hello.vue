      ...
      <draggable class="list-group" tag="ul" v-model="list" v-bind="dragOptions" :move="onMove" @start="isDragging=true" @end="isDragging=false">
        <transition-group type="transition" :name="'flip-list'">
          <li class="list-group-item" v-for="element in list" :key="element.order">
            ...
            {{element.name}}
            <span class="badge"><i class="fa fa-user"></i> {{element.order}}</span>
          </li>
        </transition-group>
      </draggable>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      list2:  messageTwo.map((name, index) => {
        return { name, order: index + 1, fixed: false };
      }),
      list3:  messageThree.map((name, index) => {
        return { name, order: index + 1, fixed: false };
      }), 
      editable: true,
      isDragging: false,
